...
export function computeStats({ weeks, marks, today = new Date() }) {
  ...
  let completed = 0, backups = 0, missed = 0, partial = 0
  let scheduledToDate = 0
  let runsDone = 0, strengthDone = 0, circuitsDone = 0

  const perPhase = {}
  for (const w of weeks) {
    const arrived = w.monday <= todayMid
  ...
  const completionRate = scheduledToDate > 0 ? completed / scheduledToDate : 0
  const backupShare = completed > 0 ? backups / completed : 0
